    ...
    public function canRead($pFilename)
    {
        ...
        if ($zipClass == 'ZipArchive') {
            $contents = $zip->getFromIndex(
                $archive->locateName($pFilename, ZIPARCHIVE::FL_NOCASE)
            );
            if ($contents === false) {
                $contents = $zip->getFromIndex(
                    $archive->locateName(substr($pFilename, 1), ZIPARCHIVE::FL_NOCASE)
                );
            }
        }
        else {
            $contents = $zip->getFromIndex($zip->locateName($pFilename));
        }
