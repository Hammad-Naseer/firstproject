...
class Sheets extends CI_Controller
{
	function __construct()
	{
        ...
        //echo phpinfo();exit;
        // if (extension_loaded('zip')) { echo '<h3>zip Load</h3>'; }
	}
	...
	function save_departments()
    {
        ...
        if($data_counter > 0) {
            ...
            $this->session->set_flashdata('error_message', 'Empty File cannot be imported!');
            ...
            foreach ($objPHPExcel->getWorksheetIterator() as $worksheet) {
                ...
                if ($sheetname == "Classes") {
                    ...
                    for ($row = 2; $row <= $highestRow; $row++) {
                        ...
                        if( (isset($dept_title) && !empty($dept_title)) && (isset($class_title) && !empty($class_title)) ) {
            ...
            foreach ($objPHPExcel->getWorksheetIterator() as $worksheet) {
                ...
                if ($sheetname == "Class Section") {
                    ...
                    for ($row = 2; $row <= $highestRow; $row++) {
                        ...
                        if( (isset($dept_title) && !empty($dept_title)) && (isset($class_title) && !empty($class_title)) && (isset($section_title) && !empty($section_title)) ) {
            ...
            foreach ($objPHPexcel->getWorksheetIterator() as $worksheet) {
                ...
                if ($sheetname == "Academic Year") {
                    ...
                    for ($row = 2; $row <= $highestRow; $row++) {
                        ...
                        $miliseconds = ($start_date - (25567 + 2)) * 86400 * 1000;
                        $seconds = $miliseconds / 1000;
                        $start_date = date("Y-m-d", $seconds);
                        
                        $miliseconds = ($end_date - (25567 + 2)) * 86400 * 1000;
                        $seconds = $miliseconds / 1000;
                        $end_date = date("Y-m-d", $seconds);
                        
                        if( (isset($title) && !empty($title)) && (isset($start_date) && !empty($start_date)) && (isset($end_date) && !empty($end_date)) && (isset($status) && !empty($status)) ) {
            ...
            foreach ($objPHPExcel->getWorksheetIterator() as $worksheet) {
                ...
                if ($sheetname == "Academic Term") {
                    ...
                    for ($row = 2; $row <= $highestRow; $row++) {
                        ...
                        if( (isset($acd_year_title) && !empty($acd_year_title)) && (isset($title) && !empty($title)) && (isset($start_date) && !empty($start_date)) && (isset($end_date) && !empty($end_date)) && (isset($status) && !empty($status)) ) {
            ...
            foreach ($objPHPExcel->getWorksheetIterator() as $worksheet) {
                ...
                if ($sheetname == "Subjects") {
                    ...
                    for ($row = 2; $row <= $highestRow; $row++) {
                        ...
                        if( (isset($sub_cat_title) && !empty($sub_cat_title)) && (isset($title) && !empty($title)) && (isset($code) && !empty($code)) ) {
            ...
            foreach ($objPHPExcel->getWorksheetIterator() as $worksheet) {
                ...
                if ($sheetname == "Designation") {
                    ...
                    for ($row = 2; $row <= $highestRow; $row++) {
                        ...
                        if( (isset($title) && !empty($title)) && (isset($is_teacher) && !empty($is_teacher)) ) {
            ...
            foreach ($objPHPExcel->getWorksheetIterator() as $worksheet) {
                ...
                if ($sheetname == "Staff") {
                    ...
                    for ($row = 2; $row <= $highestRow; $row++) {
                        ...
                        $id_no = $worksheet->getCellByColumnAndRow(2, $row)->getValue();            //CNIC
                        $employee_name = $worksheet->getCellByColumnAndRow(3, $row)->getValue();    //Employee Name
                        
                        $desg_title = $worksheet->getCellByColumnAndRow(5, $row)->getValue();       //Designation
                        $is_teacher = $worksheet->getCellByColumnAndRow(6, $row)->getValue();       //Is-Teacher
                        
                        $contact_number = $worksheet->getCellByColumnAndRow(11, $row)->getValue();  //Contact Number
                        $email = $worksheet->getCellByColumnAndRow(12, $row)->getValue();           //Email

                        if( (isset($id_no) && !empty($id_no)) && (isset($employee_name) && !empty($employee_name)) && (isset($desg_title) && !empty($desg_title)) && (isset($is_teacher) && !empty($is_teacher)) && (isset($contact_number) && !empty($contact_number)) && (isset($email) && !empty($email)) ) {
                            ...
                            $data['dob']            = date('Y-m-d', strtotime($worksheet->getCellByColumnAndRow(7, $row)->getValue()));
                            $data['gender']         = $worksheet->getCellByColumnAndRow(8, $row)->getValue();
                            $data['religion']       = $worksheet->getCellByColumnAndRow(9, $row)->getValue();
                            $data['postal_address'] = $worksheet->getCellByColumnAndRow(10, $row)->getValue();
                            $data['mobile_no']      = $contact_number;
                            $data['email']          = $email;
                            
                            if($is_teacher == 1){
                                
                               $data['periods_per_week'] = $worksheet->getCellByColumnAndRow(13, $row)->getValue();
                               $data['periods_per_day']  = $worksheet->getCellByColumnAndRow(14, $row)->getValue();
    ...
    function save_imported_students()
    {
        ...
        if (in_array($extension, $allowed_extension)) {
            ...
            $q_academic_year_id    = "SELECT academic_year_id FROM ". get_school_db() .".acadmic_year WHERE status = 2";
            $rowacademic_year_id   = $this->db->query($q_academic_year_id)->row();
            
            $q_academic_term_id    = "SELECT yearly_terms_id FROM ". get_school_db() .".yearly_terms WHERE status = 2";
            $rowacademic_term_id   = $this->db->query($q_academic_term_id)->row();
            
            
            if($rowacademic_year_id == null || $rowacademic_term_id == null)
            {
                $this->session->set_flashdata('error_message', 'There must be a current academic year and academic term');
                redirect(base_url()."sheets/import_list");  
            }
            ...
            foreach ($objPHPExcel->getWorksheetIterator() as $worksheet)
            {
                ...
                if ($sheetname == "Student Details") 
                {
                    ...
                    for ($row = 2; $row <= $highestRow; $row++)
                    {
                        ...
                        if( (isset($form_num) && !empty($form_num)) && (isset($Registration) && !empty($Registration)) && (isset($ClassName) && !empty($ClassName)) && (isset($Student_Name) && !empty($Student_Name)) && (isset($NIC) && !empty($NIC)) && (isset($Mobile) && !empty($Mobile)) && (isset($emg_num) && !empty($emg_num)) && (isset($FatherName) && !empty($FatherName)) && (isset($FatherNIC) && !empty($FatherNIC)) )
                        {
                            ...
                            $section_id             = ($details_arr[0]["section_id"] > 0) ? $details_arr[0]["section_id"] : 0;
                            $section_name           = $details_arr[0]["title"];
                            $section_departments_id = ($details_arr[0]["departments_id"] > 0) ? $details_arr[0]["departments_id"] : 0;
                            ...
                            $AcademicYearID        = $rowacademic_year_id->academic_year_id;
                            $AcademicTermID        = $rowacademic_term_id->yearly_terms_id;
                            ...
                            $updata = array( 'system_id' =>  $system_id, 'barcode_image' => $system_id.'png' );
