...
class Student_category extends CI_Controller
{
	...
	function manage_stud_category($param1 = '', $param2 = '', $param3 = '')
    {
        if ($_SESSION['user_login'] != 1)
            redirect('login');
        ...
        if ($param1 == 'create') 
        {
			...
			redirect(base_url() . 'student_category/manage_stud_category');
        }
        else if ($param1 == 'edit') 
        {
			...
			$this->db->where('student_category_id',$student_category_id);
			$this->db->update(get_school_db().'.student_category',$page_data);
        ...
        {
        	$q="select student_category_id from ".get_school_db().".student where student_category_id=$param2";
			...
			else
			{
            	...
            	$this->db->delete(get_school_db().'.student_category');
			}
        ...
        $this->db->where('school_id',$_SESSION['school_id']);
        $page_data['stud_category'] = $this->db->select("*")->from(get_school_db().'.student_category')->get()->result_array();
        $page_data['page_name'] = 'student_category_listing';
	...
	function manage_leaves_student($param1 = '', $param2 = '', $param3 = '')
    {
        ...
        if ($param1 == 'approve') {
			...
			foreach($date_leave as $row_date){
				$q="SELECT student_id FROM ".get_school_db().".attendance WHERE date='".$row_date."' AND student_id=$student_id AND school_id=".$_SESSION['school_id']."";
				$query=$this->db->query($q)->result_array();
