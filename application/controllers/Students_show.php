...
class Parents extends CI_Controller
{
	...
	function dashboard($std_id = "")
	{
		...
		//print_r($_SESSION);
		 ...
		 $qq = "select sr.*, clas.name as class_name, clas.class_id, dep.title as department_name,dep.departments_id as department_id,  c.title as section_name, sp.*,s.*
				...
				WHERE 
				sp.user_login_detail_id =".$_SESSION['login_detail_id']." 
		...
		if($qr->num_rows() > 0)
		{
			foreach($qr->result() as $qrr)
			{
				...
				$yearly_term_arr = $this->db->query("select yt.* FROM 
					".get_school_db().".acadmic_year ay
					INNER JOIN ".get_school_db().".yearly_terms yt ON ay.academic_year_id=yt.academic_year_id
					WHERE 
					 ay.school_id = ".$_SESSION['school_id']."
					AND yt.status = 2
					LIMIT 1
				")->result_array();
				/*(CURDATE() BETWEEN YT.start_date AND YT.end_date)*/
				/*AY.academic_year_id=".$_SESSION['academic_year_id']." 
					AND*/
				//$_SESSION['q'] = $this->db->last_query();
	...
	function join_virtual_class($param1 = '', $param2 = ''){
        ...
        $q = 'select * from '.get_school_db().'.class_routine JOIN '.get_school_db().'.virtual_class ON class_routine.class_routine_id = virtual_class.class_routine_id JOIN '.get_school_db().'.class_routine_settings ON class_routine.c_rout_sett_id = class_routine_settings.c_rout_sett_id WHERE class_routine_settings.section_id = '.$param2.' And class_routine.class_routine_id = '.$param1;
        $result = $this->db->query($q)->result_array();
       //print_r($result);
       foreach($result as $r){
