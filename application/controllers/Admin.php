...
class Admin extends CI_Controller
{
    ...
    function save_user_theme()
    {
        ...
        $query = $this->db->query("select * from " . get_school_db() . ".user_themes where user_login_id = $user_login_id and school_id = $school_id");
        $row =  $query->row();
        if($row != null){
            /*update*/
            $data['theme']          =   $this->input->post('theme');
            $this->db->where('user_login_id', $user_login_id);
            $this->db->update(get_school_db().'.user_themes',$data);
            echo 'Theme Updated Successfully';	
        }
        else{
            /*insert*/
            $data1['theme']          =   $this->input->post('theme');
            $data1['user_login_id']  =   $user_login_id;
            $data1['school_id']      =   $school_id;
            $this->db->insert(get_school_db().'.user_themes',$data1);
            echo 'Theme Added Successfully';	
        }
    }
    
    public function lock_screen($param="")
    {
        $page_data['last_url'] = $param;
        $page_data['page_name'] = '';
		$page_data['page_title'] = '';
        $this->load->view("backend/lock_screen",$page_data);
        $_SESSION['is_lockscreen'] = 1;
        // $last_url_str_replace = str_replace(":","/",$param);
        // $_SESSION['last_url'] = base_url().ltrim($last_url_str_replace,"/");
    }
	...
	public function lock_screen_login()
    {
        ...
        // $last_url_str_replace = str_replace(":","/",$last_url);
        $login_query = $this->db->query("select * from ".get_system_db().".user_login_details uld INNER JOIN ".get_system_db().".user_login ul ON ul.user_login_id = uld.user_login_id WHERE uld.user_login_detail_id = $login_detail_id AND ul.password = '$password' AND uld.sys_sch_id = $sys_sch_id ");
        // echo $this->db->last_query();exit;
        if($login_query->num_rows() > 0)
        {
            echo $_SESSION['last_url'];
            $_SESSION['is_lockscreen'] = 0;
        }        
    }
    ...
    function dashboard()
    {
        if(@$_SESSION['is_lockscreen'] == 1){
            redirect('Admin/lock_screen');
        }
            ...
            //count jobs count for dashboard
            $page_data['jobs_count']  = get_school_jobs_count($school_id);
            
            //count jobs count for dashboard
            $page_data['total_job_applications']  = get_school_job_application_count($school_id);
            
            //count students birtday for dashboard
            $page_data['students_birthday_count']  = get_students_birthday_count($school_id);
            
            //count staff birtday for dashboard
            $page_data['staff_birthday_count']  = get_staff_birthday_count($school_id);
            
            //count student attendance for dashboard
            $student_attendance_count = get_student_attendance_count($school_id);
            $student_attendance = array();
            foreach($student_attendance_count as $row){
                $student_attendance[$row[status]] = $row[total];
            }
            $page_data['student_attendance']  = $student_attendance;
            
            //count staff attendance for dashboard
            $staff_attendance_count = get_staff_attendance_count($school_id);
            
            // echo "<pre>";
            // print_r($staff_attendance_count);
            // exit;
            $staff_attendance = array();
            foreach($staff_attendance_count as $row){
                $staff_attendance[$row[status]] = $row[total];
            }
            $page_data['staff_attendance']  = $staff_attendance;
            
            $rating_qur = "select ROUND(AVG(tr.rating),2) as average_rating , s.name , s.staff_id from ".get_school_db().".teacher_rating tr
            inner JOIN ".get_school_db().".staff s on s.staff_id = tr.teacher_id
            WHERE tr.school_id = ".$_SESSION['school_id']." group by tr.teacher_id";
            $rating_arr = $this->db->query($rating_qur)->result();
            $page_data['rating_arr']  = $rating_arr;
            ...
            if($thm_qur->num_rows() > 0){
                $page_data['theme_val'] = $theme_query->theme;
            }
	...
	function update_parent()
	{
		...
		$page_data['std_id']=str_decode($this->uri->segment(4));
	...
	{	
		$page_data['control'] =str_decode($this->uri->segment(3));
		$page_data['std_id']=str_decode($this->uri->segment(4));
