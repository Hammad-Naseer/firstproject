...
class Depositor extends CI_Controller
{
    private $system_db;
    private $school_db;
...
*/
    public function depositor_listing($action = "", $id = 0)
    {
        ...
        if ($action == "add_edit")
        {
            ...
            $data['coa_cash_deposit'] = $this->input->post('coa_cash_deposit');
            $data['coa_bank_deposit'] = $this->input->post('coa_bank_deposit');
            $data['coa_capital_asset'] = $this->input->post('coa_capital_asset');
            ...
            if ($filename != "")
            {
                ...
                $data['attachment'] = file_upload_fun('attachment', 'depositor', 'pic');
            ...
            $designation_id = $this->input->post('designation_id');
                ...
                if ($depositor_id != "")
                {
                 ...
                 //  exit;
                    ...
                    $this->db->update(get_school_db() . '.depositor', $data);
                ...
                } else
                {
                    ...
                    $this->db->insert(get_school_db() . '.depositor', $data);

                    $depositor_id = $this->db->insert_id();
        ...
        } else if ($action == 'delete')
        {
            ...
            //} else
            {
               ...
               $qur_2 = "SELECT dpr.* FROM " . get_school_db() . ".depositor as dpr WHERE dpr.school_id = " . $_SESSION['school_id'] . " AND dpr.depositor_id = " . $id . "";
                ...
                if ($old_attachment != "")
                {
                   // exit('attachment_find');
                    $del_location = system_path($old_attachment, 'depositor');
                    file_delete($del_location);
                    $qur_1 = $this->db->query("Delete from " . get_school_db() . ".depositor where depositor_id= $id and school_id=" . $_SESSION['school_id']);
        ...
        if (isset($search_get) && ($search_get != ""))
        {
            ...
            $config['base_url'] = base_url() . "depositor/depositor_listing/" . $search_get . "";
        }
        else
        {
            $config['base_url'] = base_url() . "depositor/depositor_listing/default";
        }

        $qry_count = "SELECT count(*) as total_rows FROM " . get_school_db() . ".depositor WHERE school_id = " . $_SESSION['school_id'] . $filter_query . " ORDER BY depositor_id desc";
        ...
        $qry = "SELECT * FROM " . get_school_db() . ".depositor WHERE school_id = " . $_SESSION['school_id'] . $filter_query . " ORDER BY depositor_id desc  limit " . $start_limit . "," . $per_page . "";
        ...
        $page_data['page_name'] = 'depositor_listing';
        $page_data['page_title'] = get_phrase('depositor');
        $this->load->view('backend/index', $page_data);

    }

    function add_edit_depositor()
    {
        $page_data['page_name'] = 'depositor_add_edit';
        $page_data['page_title'] = get_phrase('depositor_add_edit');
        $this->load->view('backend/index', $page_data);
    }
