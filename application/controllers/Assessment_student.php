...
class Assessment_student extends CI_Controller
{
	...
	function assessment_result()
	{
	    ...
	    $q = "select ass.* , ass.assessment_id as ass_id , ass.total_marks as assessment_total_marks,  aud.* , r.* , r.remarks as result_remarks ,s.name as teacher_name , sub.name as subject_name ,sub.code as subject_code 
	    ...
	    left join ".get_school_db().".assessment_result r on ass.assessment_id = r.assessment_id 
	    inner join ".get_school_db().".staff s on ass.teacher_id = s.staff_id 
	    inner join ".get_school_db().".subject sub on sub.subject_id = aud.subject_id 
	    WHERE aud.student_id = ".$_SESSION['student_id']." and ass.school_id = ".$_SESSION['school_id']." order by ass.assessment_id DESC ";
