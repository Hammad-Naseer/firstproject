        ...
        <div class="login">
            ...
            <div class="wrap">
                ...
                <div class="form-wrap">
                    <!-- TABS -->
                    <div class="tabs">
                        <h3 class="login-tab"><a class="log-in active"><span>Parent Conform<span></a></h3>
                    </div>
                    <!-- TABS CONTENT -->
                    <div class="tabs-content">
                    <div class="login_error text-center">
                        <?php
                        if ($_SESSION['error_msg_exits']== 1)
                            {?>
                                <div class="form-login-error" style="display:block">
                                    
                                    <p style="color:red;">Your Account Already Exist.</p>
                                </div>
                            <?php
                            }

                        unset($_SESSION['error_msg_exits']);
                        if ($_SESSION['error_msg']== 1)
                            {?>
                                <div class="form-login-error" style="display:block">
                                    
                                    <p style="color:red;">Not found NIC and Mobile number.</p>
                                </div>
                            <?php
                            }
                    ...
                    <div id="login-tab-content" class="active">
                        <form id="my_form" name="myform" method="post" role="form" action="<?php echo base_url(); ?>signup/manage_parent_conform">
                             <input type="text" class="input form-control" oninput="get_cnic_match('f_cnic','user_login')" autofocus onkeyup="nospaces(this)" name="f_cnic" id="f_cnic" placeholder="NIC Number" onkeypress="return isNumber(event)">
                             <span class="correctNIC" style="display:none; color:white">VALID CNIC</span>
                               <span class="wrongNIC" style="display:none;color:red;width: 177px;float: left;">NIC number already exists</span>
                              <span><a href="https://gsims.com.pk/gsims/login" id="btnReset1" style="display:none;">Click to login</a></span>
                             <input type="text" class="input form-control" name="f_p_mobile" id="f_p_mobile" placeholder="Mobile Number">
                             <button type="submit" class="button btn" style="display:none;">Next</button>
                        </form>
                    </div>
                    <!-- TABS CONTENT SIGNUP -->
                    </div>
...
function nospaces(t)
{
 ...
 function get_cnic_match(cnic_n, table_name) {
        var cnic = $('#' + cnic_n).val();
        //return false;
        ...
        $.ajax({
            type: "POST",
            url: "<?php echo base_url();?>signupconform/get_cnic_check",
            dataType: 'json',
            data: {
               cnic: cnic,
            ...
            success: function(response) {
                if(response['id_no'] != cnic){
                    $(".correctNIC").css("display", "block");
                    $(".btn").css("display", "block"); //btnReset1
                    $(".wrongNIC").css("display", "none");
                    $("#btnReset1").css("display", "none");
                }else{
                    $(".correctNIC").css("display", "none");
                    $(".wrongNIC").css("display", "block");
                     $("#btnReset1").css("display", "block");
