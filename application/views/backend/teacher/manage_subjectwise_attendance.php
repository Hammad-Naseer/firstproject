...
<script>
    $(window).on("load",function() {
        ...
        <div class="row">
            <div class="col-lg-12 col-md-12 col-sm-12 myt topbar">
                ...
                <h3 class="system_name inline">
					<?php echo get_phrase('manage_subjectwise_attendance'); ?>
                </h3>
            </div>
        </div>
        <form id="student_attendance" method="post" action="<?php echo base_url();?>teacher/manage_subjectwise_attendance" class="form validate">
            <div class="">
                <div class="row filterContainer">
                    <div class="col-lg-4 col-md-4 col-sm-4" data-step="1" data-position="top" data-intro="Step 1: select date">
                    ...
                    <div class="col-lg-4 col-md-4 col-sm-4" data-step="2" data-position="top" data-intro="Step 2: class - section">
                        <select id="section_id" class="selectpicker form-control" name="section_id">
                            <?php echo section_selector($section_id);?>
                        </select>
                    </div>
                    <div class="col-lg-4 col-md-4 col-sm-4">
                        <!--<label>Selct Subject</label>-->
                        <select class="form-control" name="subject_id" id="subject_id" required >
                            <?php
                                if($section_id != ""):
                                    $subjects = get_section_subject($section_id);
                                    foreach($subjects as $row):
                                        echo '<option value="'.$row['subject_id'].'">'.$row['name'].'-'.$row['code'].'</option>';
                                    endforeach;
                                endif;
                            ?>
                        </select>
                    ...
                    <div class="col-lg-12 col-md-12 col-sm-12 mt-3" data-step="3" data-position="top" data-intro="press this button get attendance sheet">
                        ...
                        <?php if($filter > 0){?>
                        <a href="<?php echo base_url(); ?>teacher/manage_subjectwise_attendance" class="modal_cancel_btn" id="btn_remove">
		...
		<script>
         ...
         <form method="post" id="attendance_form" action="<?php echo base_url();?>teacher/apply_subjectwise_attendence/<?php echo $date.'/'.$month.'/'.$year;?>">
            <div class="row">
                ...
                <div style="overflow-y:scroll;" class="w-100">
                    <table id="fixTable" class="table table-striped table-condensed" >
                        ...
                        <tbody>
                            <?php 
                            //STUDENTS ATTENDANCE
                            $attendance="select a.date as date,s.student_id as student_id,a.status as status FROM ".get_school_db().".subjectwise_attendance a INNER JOIN ".get_school_db().".student s  ON a.student_id=s.student_id WHERE  a.school_id = ".$_SESSION['school_id']." AND s.section_id=".$section_id." AND a.subject_id=".$subject_id." and s.student_status IN (".student_query_status().")";
                ...
                <div class="col-sm-12 mgt10 text-right">
                	 <input name="submit1" type="button" id="save_attendance" class="modal_save_btn" value="<?php echo get_phrase('save_attendence'); ?>" >
                </div>  
...
<?php } ?>
        ...
        $(document).ready(function() {
            ...
            $("#section_id").on('change ' , function(){
                section_id = $(this).val();
                $.ajax ({ 
                    url:"<?= base_url() ?>Attendance/get_subjects_from_section_id",
                    type:"POST",
                    data: {
