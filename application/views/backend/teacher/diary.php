...
<div class="row">
    <div class="col-md-12 col-sm-12">
        <a href="<?php echo base_url();?>teacher/add_edit_diary/" class="modal_save_btn float-right">Add Diary</a>
    </div>
</div>
 <div class="row dairy-row-set">
    <div class="col-sm-12 col-md-12 col-lg-3 diary-left pr-5">
            ...
            <div class="diary-subjects"> 
                <?php
                 foreach ($teacher_section as $row){
                    $section_detail = section_hierarchy($row);
                ?>
                <div class="diary-tree">
                    <?php echo $row;?>
                    <ul class="diary-ul"> 
                        <li>
                            <span onclick="get_section_subjects('<?php echo $row;?>')">
                                <a style="color:#000; text-decoration:none;" data-toggle="collapse" href="#subjects<?php echo $row;?>" aria-expanded="true" aria-controls="subjects">
                                    <i class="collapsed">
                            ...
                            <div id="subjects<?php echo $row;?>" class="collapse">
                                <ul class="subjects<?php echo $row;?>"></ul>
                            </div>
                        </li>
                    </ul>  
                </div> 
...
<script type="text/javascript">
    ...
    function get_section_subjects(section_id) {
      $.ajax ({ 
          url:"<?= base_url() ?>teacher/get_section_subjects",
          type:"POST",
          data: {
    ...
    function getsubjectdiary(subject_id,section_id) {
       $.ajax ({ 
           url:"<?= base_url() ?>teacher/getsubjectdiary",
           type:"POST",
           data: {
    ...
    function get_diary_data(diary_id) {
       ...
       $.ajax ({ 
           url:"<?= base_url() ?>teacher/get_diary_data",
           type:"POST",
           data: {
               diary_id : diary_id,
           },
