...
//echo "<pre>";print_r($std_rec);
 $PassDOB = $std_rec['birthday'];
if($std_rec['image']!=""){
...
<?php 
if($_SESSION['std_create'] == 1)
{ ?>
    <div class="form-login-error" style="display:block">
        
        <p style="color:green;">Student Login Created Successfully</p>
    </div>
  <?php
} 
unset($_SESSION['std_create']);
if($_SESSION['std_update'] == 1)
  { ?>
    <div class="form-login-error" style="display:block">
        
        <p style="color:green;">Student Login Updated Successfully</p>
    </div>
  <?php 
} 
unset($_SESSION['std_update']);
?>
</div>
<script>  
...
<?php echo form_open(base_url().'parents/manage_student_login/'.$this->uri->segment(3).'/'.$this->uri->segment(4), array(
'id'=>'student_update',
'class' => 'form-horizontal form-groups-bordered validate',  'onsubmit="return validateform()"','enctype' => 'multipart/form-data'));?>                 
<?php
...
<?php
 $nicId = $std_rec["id_no"];
    ...
    if($std_rec['password'] == ''){
     ...
     $pass =   $std_rec['password'];   
    }
?>
 
<div class="form-group">
<div class="col-sm-5">
<input required="" maxlength="100" id="p_name" type="hidden" class="form-control" name="f_p_name" value="<?php echo $std_rec['name'];  ?>">
</div>
</div>
...
<div class="form-group">
			...
			<input maxlength="50" type="password" class="form-control" id="pass" name="password" value="<?php echo $pass; ?>">
			            <span id="check" class="btn btn-default">show password</span>
			            <span id="check1" class="btn btn-default" style="display:none;">hide password</span>
						</div>
					</div>	
