...
<?php
    ...
    $month = date( 'm');
    $year = date( 'Y');
    $qur_c = $this->db->query( "select  count(s.student_id) as total_count , a.date from " . get_school_db() . ".attendance a 
    ...
    where  a.status=1 and a.school_id=$d_school_id and month(date)=$month  and year(date)=$year $section_id_count group by date" )->result_array();
	...
	$month = date( 'm');
	$year = date( 'Y');
	$qur_a = $this->db->query( "select  count(s.student_id) as total_count , a.date from " . get_school_db() . ".attendance a 
	inner join " . get_school_db() . ".student s on s.student_id=a.student_id
	inner join " . get_school_db() . ".class_section cs on s.section_id=cs.section_id
	where  a.status=2 and a.school_id=$d_school_id and month(date)=$month and year(date)=$year $section_id_count group by date" )->result_array();
	...
	$month = date( 'm');
	$year = date( 'Y');
	$qur_l = $this->db->query( "select  count(s.student_id) as total_count , a.date from " . get_school_db() . ".attendance a 
	inner join " . get_school_db() . ".student s on s.student_id=a.student_id
	inner join " . get_school_db() . ".class_section cs on s.section_id=cs.section_id
	where  a.status=3 and a.school_id=$d_school_id and month(date)=$month and year(date)=$year $section_id_count
	...
	?>
	<div id="attandance_class"></div>
	</div>
	<script>
		Highcharts.chart( 'attandance_class', {
			chart: {
				type: 'column'
			},
			title: {
