...
$qur_c = $this->db->query( "select  count(s.staff_id) as total_count , a.status from " . get_school_db() .
 ".attendance_staff a inner join " . get_school_db() . ".staff s on s.staff_id=a.staff_id where a.school_id=$d_school_id and date=date(now()) group by  a.status" )->result_array();
$present_ary = array();
...
$qur_c = $this->db->query( "select  a.status , s.name ,s.staff_id from " . get_school_db() .".attendance_staff a  inner join " . get_school_db() . ".staff s on s.staff_id=a.staff_id where a.school_id=$d_school_id and date=date(now()) group by  s.staff_id" )->result_array();
$present_ary=array();
...
?>
<div id="staff_daily_chart_home" style="min-width: 310px; max-width: 600px; height: 400px; margin: 0 auto"></div>
				</div>
				
<script>
// Create the chart
Highcharts.chart('staff_daily_chart_home', {
    chart: {
        type: 'pie'
    },
    title: {
