...
<div id="header" style="height: 100px; width: 100%;">
    <div style="float: left; margin-top: -15px; height: 100px; width: 150px;"> 
        ...
        $d_school_id = $_SESSION['school_id'];
        {
            ...
            {
            ?>
            <a href=""><img style="width: 150px; height: 100px; margin-top: -15px;" src="<?php //echo base_url();?>assets/images/gsims_logo.png">
            </a>
            <?php
            }
            else
		...
		<table>
			<tr>
				<td>
	            	...
	            	<?php echo $bank_receipt_detail_arr[0]['voucher_number'];?>
            	</td>
				<td>
					<strong>
	            	...
	            	<?php 
	            		echo date_view($bank_receipt_detail_arr[0]['voucher_date']);
	            	?>
				</td>
			</tr>
			<tr>
				<td>
	            	...
	            	<?php
	            	$depositor_detail = get_depositor_details($bank_receipt_detail_arr[0]['depositor_id']);
				...
				<td>
	            	...
	            	<?php echo date_view($bank_receipt_detail_arr[0]['date_posted']);?>
				</td>
			</tr>
		</table>
</div>
...
<div id="table_data" style="margin-top: 20px;">
    ...
    <thead>
    <tr>
    <th style="width: 25px;"><?php echo get_phrase('method');?></th>
    <th><?php echo get_phrase('deposit_slip_number');?></th>
    <th><?php echo get_phrase('description');?></th>
    <th style="width: 65px;"><?php echo get_phrase('deposit_bank');?></th>
    <th><?php echo get_phrase('amount');?></th>
    <th><?php echo get_phrase('attachment');?></th>
    </tr>
    </thead>
    <tbody>
    <?php
    $total = 0;
    foreach ($bank_receipt_detail_arr as $key => $value)
    {
    ?>
    <tr>
        <td><?php echo method_check($value['method']);?></td>
        <td><?php echo $value['deposit_slip_number'];?></td>
        <td><?php echo $value['description'];?></td>
        <td>
        <?php 
        	$bank_detail = get_bank_details($value['deposit_bank_id']);
        	echo $bank_detail[0]['bank_name'];
        ?>
        </td>
        <td>
	        <?php
    ...
    ?>
    <tr>
        <td colspan="4" style="text-align: right;"><?php echo get_phrase('total');?></td>
    ...
    <?php
        if (empty($bank_receipt_detail_arr)) {
        ?>
	...
	<table>
		...
		<tr>
			...
			<?php 
				...
				<?php
				$user_detail = get_user_login_detail($bank_receipt_detail_arr[0]['submitted_by']);
			...
			<?php
				...
				<?php
				$user_detail = get_user_login_detail($bank_receipt_detail_arr[0]['posted_by']);
				echo $user_detail['name'];
				?>
			</td>
		</tr>
