...
<div class="tab-pane box active" id="edit" style="padding: 5px">
    <div class="box-content">
        ...
        <?php echo form_open(base_url().'admin/invoice/do_update/'.$row['invoice_id'], array('id'=>'disable_submit_btn','class' => 'form-horizontal form-groups-bordered validate','target'=>'_top'));?>
                <div class="form-group">
                    ...
                    <div class="col-sm-8">
                        <select name="student_id" class="form-control" style="width:400px;" >
                            ...
                            $students = $this->db->get(get_school_db().'.student')->result_array();
                            foreach($students as $row2):
                            ?>
                                <option value="<?php echo $row2['student_id'];?>"
                                    <?php if($row['student_id']==$row2['student_id'])echo 'selected';?>>
                                    <?php echo get_phrase('class');?> <?php echo $this->crud_model->get_class_name($row2['class_id']);?> -
                                    <?php echo get_phrase('roll');?> <?php echo $row2['roll'];?> -
                                    <?php echo $row2['name'];?>
                                </option>
                            <?php
                            endforeach;
